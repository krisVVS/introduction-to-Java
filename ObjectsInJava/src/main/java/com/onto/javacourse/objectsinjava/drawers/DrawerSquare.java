...
/**
 * The {@code DrawerSquare} class implements the {@code Drawer} interface for drawing {@code Square}
 * objects using an {@code OntoRobot}. This class contains the logic to draw a square on a grid by
 * placing beepers at the corners and along the edges according to the specified width.
 */
public class DrawerSquare implements Drawer<Square> {

  /**
   * Draws a square using the specified {@code Square} object and {@code OntoRobot}. The robot moves
   * to the starting point of the square and places beepers along its edges based on the square's
   * dimensions.
   *
   * @param square the {@code Square} object that defines the dimensions and position of the square
   *     to draw
